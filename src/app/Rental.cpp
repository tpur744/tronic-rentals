...
Rental::Rental(const std::string &registration_plate, const std::string &make,
    ...
    : Car(registration_plate, make, daily_rental_fee), 
      ...
      customer_id_(customer_id), rental_reference_(rental_reference) {}

    


std::string Rental::GetStartDate() const {
  return start_date_.ToString();
}

std::string Rental::GetEndDate() const {
  return end_date_.ToString();
}
...
std::string Rental::GetRentalReference() const {
    return rental_reference_;
}

std::string Rental::GetNumberPlate() const {
    return Car::GetNumberPlate(); // Call base class method or provide a new implementation
}
